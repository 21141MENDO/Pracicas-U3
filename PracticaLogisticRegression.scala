...
val logredataall = (timedata.select(data("Clicked on Ad").as("label"), $"Daily Time Spent on Site", $"Age", $"Area Income", $"Daily Internet Usage", $"Timestamp", $"Male"))
...
// Cree un nuevo objecto VectorAssembler llamado assembler para los featureval timedata = data.withcolumn("Hour", hour(data("Timestamp")))
